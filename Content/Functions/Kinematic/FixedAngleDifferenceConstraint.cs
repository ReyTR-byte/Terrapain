...
namespace Terrapain.Content.TUtilities.Kinematic
{
    public class FixedAngleDifferenceConstraint(float maxAngleDifference, float idealAngle) : IJointConstraint
    {
        ...
        public double ApplyPenaltyLoss(Joint owner, float gradientDescentCompletion)
        {
            ...
            float penaltyFactor = Utilities.InverseLerp(0.4f, 0f, gradientDescentCompletion) * 1500f;
            float penalty = Utilities.InverseLerp(MaxAngleDifference, MathF.PI, jointDirection.AngleBetween(IdealAngle.ToRotationVector2())) * penaltyFactor;
