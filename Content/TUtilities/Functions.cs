    ...
    //            Chatic((int)(rotation / ((float)Math.PI * 2) + i) * (float)Math.PI * 2);
        ...
        public static bool CheckActiveAccessories(Player player)
		{
			for (int i = 0; i < 7; i++)
			{
				...
				if (item.active && ((item.GetGlobalItem<TGlobalItem>().ActiveAccessory != null && (!item.GetGlobalItem<TGlobalItem>().ActiveAccessory.AbilityUnarmedOnly || player.GetModPlayer<TerrapainPlayer>().unarmed)) || item.GetGlobalItem<TGlobalItem>().dashAccessory))
                    return true;
            }
			return false;
		}
        public static List<Item> GetActiveAccessories(Player player)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
				...
				if (player.armor[i + 3].active && player.armor[i + 3].GetGlobalItem<TGlobalItem>().activeAccessory && (!player.armor[i + 3].GetGlobalItem<TGlobalItem>().ActiveAccessory.AbilityUnarmedOnly || player.GetModPlayer<TerrapainPlayer>().unarmed))
				{
